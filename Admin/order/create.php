<?php 
  ...
  $sql = "select books.price , cart.id from books inner join cart on books.id = cart.book_id where cart.id = $id";
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (count($errors) > 0) {
        ...
        // db ..........
        $sql  = "insert into orders (name,account,credit,full_price,status,user_id,cart_id) values ('$name','$user_name','$credit',$price,'pending',$user_id,$id)";
        ...
        if ($op) {
            ...
            $order = mysqli_fetch_assoc($op);
            $order_id = $order['id'];

            $sql = "insert into details (count ,titles,client) values ($count,'$title','$user_name')";

            $opd = mysqli_query($con,$sql);
